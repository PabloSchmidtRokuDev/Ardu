...
void ColorFijo() {
 ...
 // colorX ccl = colorXList[random(0, 52)];
  ...
  //ccl = colorXList[random(0, 52)];
  LED2_Brillo_value[0] = random(0, 255);
  LED2_Brillo_value[1] = random(0, 255);
  LED2_Brillo_value[2] = random(0, 255);
  LED2_Brillo_value[3] = random(10, 50);
}
...
void setLedColor() {
  ...
  if (!bolaPowerOn) {
    ...
    //  Serial.println("setLedColor cambio color");
    ...
    while (flag == 0) {
      for (int i = 0; i < 4; i++) {
        ...
        if (LED2_Current_value[i] < LED2_Brillo_value[i]) {
          LED2_Current_value[i]++;
          if (i == 0) analogWrite(LED2_Pins[i], map(LED2_Current_value[0], 0, 255, 0, 20));
          else
            analogWrite(LED2_Pins[i], LED2_Current_value[i]);
  ...
  // Serial.println("setLedColor cambio color fin");
  // return flag;
}
